...
axios.get( `${siteUrl}/wp-json/custom-login-api/v1/nonce`)
  .then( response => {
  if ( undefined === response.data.nonce_action || undefined === response.data.nonce_value){
    setInputData({
    ...inputData,
  ...
  return axios.post( `${siteUrl}/wp-json/custom-login-api/v1/login` , finalData)
  })
    .then( response => {
    const nonce_action = localStorage.getItem( "nonce_action" );
    const nonce_value = localStorage.getItem( "nonce_value" );
    const author_id = localStorage.setItem( "author_id" ,response.data.author_id );
    setInputData( {
      ...inputData,
      nonce_action: nonce_action,
      nonce_value : nonce_value,
